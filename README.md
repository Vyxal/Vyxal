...
Vyxal is written in Python, which isn't particularly fast in the first place. Additionally, a `ctx` object is passed between every function to keep the global state clean (because Python), which probably causes a bit of lag.

Vyxal's fine for smaller programs, but will take large amounts of time for anything complex. If you wish to run Vyxal faster, see [Jyxal](https://github.com/Vyxal/Jyxal), a work-in-progress Vyxal interpreter that compiles Vyxal into JVM bytecode for high speed.

Vyxal is not a good choice for a large project due to lack of a module system or any way of combining multiple files. These may be added at some point in the future.
