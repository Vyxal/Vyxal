...
/** Generates nanorc files for both SBCS and literate modes, returning a mapping
  * from each file's name to its contents
  */
@main def generateNanorc(sbcsFile: String, litFile: String) =
  val sbcs = commonHeader + commonFooter
  val lit = commonHeader + litSpecific + commonFooter
  Files.write(Paths.get(sbcsFile), sbcs.getBytes(StandardCharsets.UTF_8))
  Files.write(Paths.get(litFile), lit.getBytes(StandardCharsets.UTF_8))
