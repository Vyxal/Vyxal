...
object MiscHelpers:
  ...
  val absoluteDifference: Dyad = Dyad.fill("absoluteDifference") {
    case (a: VNum, b: VNum) => (a - b).vabs
    case (a: VList, b: VNum) => a.vmap(MiscHelpers.absoluteDifference(_, b))
    case (a: VNum, b: VList) => b.vmap(MiscHelpers.absoluteDifference(a, _))
    case (a: VList, b: VList) => a.zipWith(b)(MiscHelpers.absoluteDifference)
    case (a: VList, b: VFun) =>
