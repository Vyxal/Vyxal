...
import Varser.reservedTypes
import VarsingException.*

case class VarserResult(
    ast: AST,
...
object Varser:
  @throws[VarsingException]
  def parse(tokens: List[Token]): VarserResult =
    val parser = Varser()
...
enum VarsingException(msg: String) extends VyxalException(msg):
  case BadAugmentedAssignException()
      extends VarsingException("Missing element for augmented assign")
  case BadModifierException(modifier: String)
      extends VarsingException(
        s"Modifier '$modifier' is missing arguments"
      )
  case BadStructureException(structure: String)
      extends VarsingException(s"Invalid $structure statement")
  ...
  case NoSuchElementException(element: String)
      extends VarsingException(s"No such element: $element")
  case TokensFailedParsingException(tokens: List[Token])
      extends VarsingException(s"Some elements failed to parse: $tokens")
  ...
  case UndefinedCustomModifierException(modifier: String)
      extends VarsingException(s"Custom modifier '$modifier' not defined")

  case UndefinedCustomElementException(element: String)
      extends VarsingException(s"Custom element '$element' not defined")
