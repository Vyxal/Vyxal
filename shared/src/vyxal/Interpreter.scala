      ...
      case AST.Command(cmd, _, overwriteable) =>
        ...
        if !executed then
          Elements.elements.get(cmd) match
            ...
            case None => Elements.internalUseElements.get(cmd) match
                case Some(elem) => elem.impl()
                case None =>
      ...
      case AST.TryCatch(success, error, _) =>
        ...
        try
          ...
          case ex: VyxalException =>
            ...
            ctx.clear()
            ctx.push(savedStack*)
            ctx.push(VObject(ex.getClass.getSimpleName, MiscHelpers.getConstructorParams(ex) + ("message" -> (Visibility.Restricted, VStr(ex.getMessage())))))
            execute(error)
