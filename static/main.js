...
const other_aliases = {
    ...
    "⋏": [/bit(wise)?_?and/,/pad_?c(enter)?|c(enter)?_?pad/],
    "⋎": [/bit(wise)?_?or/,/(rm|remove)_?ind(ex)?/,"merge"],
    "꘍": [/bitwise_?xor/,/l(even(shtein)?)?_?dist(ance)?/,/(add|prepend|append)_?spaces?/],
    "ꜝ": [/bitwise_?not/,/filter_?fals[ye]/,/any_?upper(case)?/],
    "℅": ["choice"],
    "Þ℅": ["shuffle"],
    "≤": [/le(ss)?_?eq(ual)?/],
    "≥": [/gr(eater)?_?eq(ual)?/],
    ...
    "‡": [/l(am(bda)?)?_?2|2_?l(am(bda)?)?/],
    "≬": [/l(am(bda)?)?_?3|3_?l(am(bda)?)?/],
