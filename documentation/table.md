...
| <code>Ϣ</code> | <code>chunk-to-length</code></br><code>partition-to-length</code></br><code>first-n-true</code> | 2 |   | **Chunk to Length** (`any,num\|str`): Chunk #1 into parts of length #2</br>**Chunk to Length** (`num\|str,any`): Chunk #2 into parts of length #1</br>**Partition to Lengths** (`itr,lst[num]`): Partition #1 into parts of lengths #2</br>**First N Integers Where Function is True** (`num,fun`): First #1 integers where function #2 is true</br>**First N Integers Where Function is True** (`fun,num`): First #2 integers where function #1 is true |
...
| <code>Þ⊖</code> | <code>reg-pop-n</code></br><code>register-pop-n</code> | 1 |   | **Pop n from Register** (`num`): Pop the first #1 items from the register to the stack as a list |
