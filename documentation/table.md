...
| <code>£</code> | <code>set-register</code> | 1 |   | **Set Register** (`any`): Set the register to #1 |
| <code>¥</code> | <code>get-register</code></br><code>push-register</code> | 0 |   | **Get Register**: Push the register to the stack |
...
| <code>↻</code> | <code>rot-right</code></br><code>iterate-unique-length</code> | 1 |   | **Rotate Right** (`lst\|str`): Rotate #1 right</br>**Rotate Right** (`lst\|str,num`): Rotate #1 right #2 times. Left if #2 is negative</br>**Iterate while Unique** (`any,fun`): Repeatedly apply #2 to #1 until a result is repeated, return the length |
...
| <code>∻</code> | <code>integer-divide</code></br><code>int-div</code></br><code>//</code></br><code>overwrite</code></br><code>vertical-merge</code> | 2 | vec  | **Integer Divide** (`num,num`): #1 // #2</br>**Vertical Merge** (`str,str`): overwrite the start of the shorter string with the longer one |
...
| <code>⎘</code> | <code>flatten-by-depth</code></br><code>flatten-depth</code></br><code>one-flatten</code> | 2 |   | **Flatten by Depth** (`lst,num`): Flatten #1 by #2 levels</br>**Flatten by Depth** (`lst`): Flatten #1 by 1 level</br>**Flatten-each** (`lst[num\|str]`): Flattens each item in #1 |
