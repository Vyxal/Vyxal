...
| <code>r</code> | <code>replace</code> | 3 |   | **Replace** (`nsl,nsl,nsl`): Replace all occurrences of #2 in #1 with #3</br>**Zip-With** (`lst,lst,fun`): Zip #1 and #2 and apply #3 to each pair</br>**Zip-With** (`lst,fun,lst`): Zip #1 and #2 and apply #3 to each pair</br>**Zip-With** (`fun,lst,lst`): Zip #1 and #2 and apply #3 to each pair |
...
| <code>≤</code> | <code>less-than-or-equal</code></br><code>lte</code></br><code><=</code> | 2 | vec  | **Less Than or Equal** (`scl,scl`): #1 <= #2 |
| <code>≥</code> | <code>greater-than-or-equal</code></br><code>gte</code></br><code>>=</code> | 2 | vec  | **Greater Than or Equal** (`scl,scl`): #1 >= #2 |
...
| <code>„</code> | <code>join-on-spaces</code></br><code>*space</code></br><code><0</code></br><code>is-negative?</code></br><code>intesperse-spaces</code> | 1 |   | **Join on Spaces** (`lst`): Join #1 on spaces</br>**Is negative?** (`num`): Push 1 if #1 < 0, 0 otherwise</br>**Intersperse #1 with spaces** (`str`): Insert spaces between each character of #1 |
