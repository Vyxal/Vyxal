...
| <code>K</code> | <code>factors</code></br><code>is-numeric?</code></br><code>is-numeric</code> | 1 | vec  | **Factors** (`num`): Get the factors of #1</br>**Is Numeric** (`str`): Check if #1 is numeric |
...
| <code>V</code> | <code>vectorse-reverse</code></br><code>1-x</code> | 1 |   | **Vectorise Reverse** (`lst`): Reverse each item in #1</br>**1 - X** (`num`): 1 - #1 |
...
| <code>d</code> | <code>double</code> | 1 |   | **Double** (`num`): #1 * 2</br>**Double** (`str`): Append a copy of #1 to itself |
...
| <code>n</code> | <code>ctx-primary</code></br><code>ctx</code></br><code>ctx-n</code></br><code>context-n</code></br><code>context-primary</code> | 0 |   | **Context Primary**: Push the primary context variable to the stack |
...
| <code>w</code> | <code>wrap-in-list</code> | 1 |   | **Wrap in List** (`any`): Wrap #1 in a list |
...
| <code>ᐐ</code> | <code>init</code> | 1 |   | **Init** (`any`): All but the last element of #1 |
...
| <code>ᐕ</code> | <code>behead</code> | 1 |   | **Behead** (`any`): All but the first element of #1 |
...
| <code>⧢</code> | <code>into-n-pieces</code></br><code>split-into-n-pieces</code></br><code>fixpoint-collect-tail</code> | 2 |   | **Split Into N Pieces** (`itr,num`): Split #1 into #2 pieces</br>**Split Into N Pieces** (`num,itr`): Split #2 into #1 pieces</br>**Split Number Into N Pieces** (`itr,num`): Listify {#1|#2} and split it into {#2|#1} pieces</br>**Fixpoint Unfold Without Initial Value** (`fun,any`): Repeatedly apply #1 to #2, collecting results (not including initial value)</br>**Fixpoint Unfold Without Initial Value** (`any,fun`): Repeatedly apply #2 to #1, collecting results (not including initial value) |
