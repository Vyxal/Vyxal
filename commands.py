...
""", 2),
    ...
    "øo": ("needle, haystack = pop(stack, 2); stack.append(infinite_replace(haystack, needle, ''))", 2),
    "øV": ("replacement, needle, haystack = pop(stack, 3); stack.append(infinite_replace(haystack, needle, replacement))", 3),
    "øc": ("value = pop(stack); stack.append('«' + utilities.from_ten(utilities.to_ten(value, utilities.base27alphabet), encoding.codepage_string_compress) + '«')", 1),
    "øC": ("number = pop(stack); stack.append('»' + utilities.from_ten(number, encoding.codepage_number_compress) + '»')", 1),
    ...
    "øṙ": ("replacent, pattern, source = pop(stack, 3); stack.append(regex_replace(VY_str(source), VY_str(pattern), replacent))", 3),
    "øp": ("rhs, lhs = pop(stack, 2); stack.append(int(str(lhs).startswith(str(rhs))))", 2),
    ...
    "Þ↓": ("fn, vector = pop(stack, 2); stack.append(min(VY_zipmap(fn, vector), key=lambda x: x[-1])[0])", 2),
    "Þ↑": ("fn, vector = pop(stack, 2); stack.append(max(VY_zipmap(fn, vector), key=lambda x: x[-1])[0])", 2),
    ...
    "ÞṪ": ("rhs, lhs = pop(stack, 2); print(lhs, rhs) ;stack.append(Generator(itertools.zip_longest(*iterable(lhs), fillvalue=rhs)))", 2),
    "Þ℅": ("top = iterable(pop(stack)); stack.append(random.sample(top, len(top)))", 1),
    "Þ•": ("rhs, lhs = pop(stack, 2); stack.append(dot_product(iterable(lhs), iterable(rhs)))", 2),
    "ÞṀ": ("rhs, lhs = pop(stack, 2); stack.append(matrix_multiply(iterable(lhs), iterable(rhs)))", 2),
    ...
    "ÞR": ("fn, vector = pop(stack, 2); stack.append(foldl_rows(fn, deref(vector)))", 2),
    "ÞC": ("fn, vector = pop(stack, 2); stack.append(foldl_cols(fn, deref(vector)))", 2),
    ...
    "kð": ("stack.append([date.today().day, date.today().month, date.today().year])", 0),
    "kβ": ("stack.append('{}[]<>()')", 0),
    "kḂ": ("stack.append('()[]{}')", 0),
    "kß": ("stack.append('()[]')", 0),
    "kḃ": ("stack.append('([{')", 0),
    ...
    "k§": ("stack.append(['Monday', 'Tuesday', 'Wednesday', 'Thursday', 'Friday', 'Saturday', 'Sunday'])", 0),
